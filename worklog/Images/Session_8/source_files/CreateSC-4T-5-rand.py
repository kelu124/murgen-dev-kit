...
for i in range(size[0]): # les lignes
	...
	pix[i,j]=128
	value = 0
	value = SortedTable[i][j*DECIMATION+0]*(1.9*(j*DECIMATION+k)**(ATTENUATION_COEF)/(Depth**(ATTENUATION_COEF)))
	value = int(value)
	ImagePoints[i][j] = (int)(value/64)
        pix[i,j] = (ImagePoints[i][j],ImagePoints[i][j],ImagePoints[i][j]) 


outfile = startingpoint +"-DEC"+str(DECIMATION)+"-rand.png"
im.save(outfile)
...
if True: #comment
	...
	for i in range(MaxDepth):
	    ...
	    print i 
	outfile = startingpoint +"-DEC"+str(DECIMATION)+"-SC-4T-rand.png"
	im.save(outfile)

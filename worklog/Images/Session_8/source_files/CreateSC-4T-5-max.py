...
for i in range(size[0]): # les lignes
	...
	for k in range(DECIMATION):
		PixelValue = SortedTable[i][j*DECIMATION+k]*(1.9*(j*DECIMATION+k)**(ATTENUATION_COEF)/(Depth**(ATTENUATION_COEF)))
		if value < PixelValue:
			value = PixelValue
	value = int(value)
...
outfile = startingpoint +"-DEC"+str(DECIMATION)+"-max.png"
im.save(outfile)
...
if True: #comment
	...
	for i in range(MaxDepth):
	    ...
	    print i 
	outfile = startingpoint +"-DEC"+str(DECIMATION)+"-SC-4T-max.png"
	im.save(outfile)
